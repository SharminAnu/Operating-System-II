...
int  main(int argc, char *argv[])
{
    ...
    if (getcontext(&rtn_env) == -1)
    {
        perror("getcontext in main");
        exit(5);
    }

    rtn_env.uc_stack.ss_sp = (void *)(malloc(STACKSIZE));
    if (rtn_env.uc_stack.ss_sp == NULL)
    {
        printf("End Game failed to get stack space\n");
        exit(8);
    }

    rtn_env.uc_stack.ss_size = STACKSIZE;
    rtn_env.uc_stack.ss_flags = 0;

    makecontext(&rtn_env,end_game,0);
...
void end_game(void)
{
    ...
    printf("#### Thread %d hit end game: stack at %x", current, &marker);
    clock_isr(-current);
...
int    t_init (void (*fun_addr)(), int fun_arg)
{
    ...
    t_state[i].run_env.uc_link   = &rtn_env;
